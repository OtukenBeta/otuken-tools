...
echo -e "${BLUE}                                    https://github.com/alphaotuken/otuken-tools ${NC}"
...
install_dir="/usr/share/otuken-tools"
bin_dir="/usr/bin"
...
if [[ $choice =~ ^[1-2]+$ ]]; then
    ...
    if curl -s -m 10 https://www.google.com > /dev/null || curl -s -m 10 https://www.github.com > /dev/null; then
        ...
        echo "";
        echo -e "${YELLOW}[✔] Downloading otuken-tools...${NC}"
        if sudo git clone https://github.com/alphaotuken/otuken-tools.git $install_dir; then
...
#            echo '#!/bin/bash' > hackingtool.sh
            ...
            echo -e "${RED}[✘] Failed to download otuken-tools [✘]"
            exit 1
        fi
    ...
    if [ -d $install_dir ]; then
        ...
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]     ✔✔✔ Now Just Type In Terminal (otuken-tools) ✔✔✔      [+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
    else
